...
const Cover = () => (
  <section
    data-testid="brochure-cover"
    className="relative min-h-[100svh] overflow-hidden bg-night text-ivory"
  >
    <ParallaxImage
      image={heroImage}
      priority
    ...
    <div
      className="absolute inset-x-0 bottom-0 h-3/5 bg-gradient-to-t from-night via-night/50 to-transparent"
      aria-hidden="true"
    />
    ...
    <div className="container-x relative z-10 flex min-h-[100svh] flex-col justify-between pb-[calc(env(safe-area-inset-bottom)+6.5rem)] pt-32 sm:pb-20 lg:pt-40">
      <Reveal className="flex items-center justify-between sm:justify-between">
        ...
        <span className="label text-ivory/55">Studio Brochure · {YEAR}</span>
      </Reveal>
      ...
      <div>
        ...
        <SplitLines
          ...
          lines={[
            "Interiors that",
            <>
              feel like <span className="italic text-brass">home.</span>
            </>,
          ]}
          className="h-display mt-7 text-[clamp(3rem,min(11vw,16svh),7.5rem)]"
        />
        <div className="mt-10 grid items-end gap-8 lg:grid-cols-12">
          <Reveal delay={0.5} className="lg:col-span-5">
            <p className="max-w-md text-base leading-[1.7] text-ivory/80 md:text-[17px]">
              {site.positioning}
            </p>
          </Reveal>
          <Reveal
            delay={0.6}
            className="flex flex-wrap items-center gap-x-8 gap-y-5 lg:col-span-7 lg:justify-end"
          >
            <DownloadButton className="btn-brand" />
            <Link
              to="/contact"
              data-testid="brochure-cover-contact"
              className="btn-text-light text-[11px]"
            >
              Start a Project{" "}
  ...
  >
    <div className="container-x flex h-14 items-center justify-between gap-4">
      <p className="label flex items-center gap-3 whitespace-nowrap text-taupe">
        ...
        <span className="hidden sm:inline">Suvi Interior · </span>Brochure{" "}
        {YEAR}
      </p>
        ...
        <div className="lg:col-span-6">
          <Head
            ...
            title={[
              "Designing",
              "spaces that",
              <span className="italic">feel like you.</span>,
            ]}
          />
          <Reveal delay={0.25} className="mt-10 flex flex-col gap-6 lg:mt-14">
            ...
            {[...story.body.slice(0, 1), ...philosophy.body].map((p) => (
              <p
                key={p}
                className="text-[15px] leading-[1.75] text-charcoal/80"
              >
                {p}
              </p>
            ))}
          ...
          <Reveal delay={0.3} className="mt-12">
            ...
            <ol className="mt-4 border-t border-line">
              {principles.map((p) => (
                <li
                  key={p.n}
                  className="grid grid-cols-[3rem_1fr] gap-4 border-b border-line py-5 sm:grid-cols-[5rem_1fr]"
                >
...
const Capabilities = () => (
  <section
    data-testid="brochure-capabilities"
    ...
    <div className="container-x section">
      <div className="grid gap-10 lg:grid-cols-12">
        <Head
          index="02"
          label="Capabilities"
          title={["What we", <span className="italic">design & make.</span>]}
        ...
        >
          <p className="lede">
            Six ways of working with the studio — from a single made-to-measure
            piece to a complete home, designed and manufactured under one roof.
          </p>
        </Reveal>
      </div>
      ...
      <ol className="mt-16 grid gap-x-8 gap-y-16 md:grid-cols-2 lg:mt-24">
        {services.map((s, i) => (
          <Reveal
            ...
            as="li"
            delay={(i % 2) * 0.1}
            data-testid={`brochure-service-${s.slug}`}
          ...
          >
            ...
            <p className="mt-4 max-w-lg text-[15px] leading-[1.75] text-charcoal/80">
              {s.description}
            </p>
                ...
                <ul className="mt-3 flex flex-col gap-1.5 text-sm text-charcoal/85">
                  {s.features.map((f) => (
                    <li key={f} className="flex items-baseline gap-3">
                      <span
                        className="h-px w-3 shrink-0 translate-y-[-3px] bg-brass"
                        aria-hidden="true"
                      />
                      {f}
                    </li>
    ...
    <div className="container-x section">
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        ...
        <Reveal delay={0.15} className="md:pb-2">
          <Link
            to="/projects"
            data-testid="brochure-projects-link"
            className="btn-text"
          >
...
export default function Brochure() {
  return (
    <PageWrap theme="light" testId="brochure-page">
        ...
        <Studio index="06" />
        <section
          className="border-t border-line bg-sand no-print"
          data-testid="brochure-download-band"
        >
          <div className="container-x flex flex-col gap-8 py-16 md:flex-row md:items-center md:justify-between md:py-20">
            <div>
              ...
              <p className="h-statement mt-5 max-w-xl">
                Take the studio with you — a seven-page{" "}
                <span className="italic">A4 brochure</span> to read, share or
                print.
              </p>
            </div>
            <DownloadButton
              className="btn-solid self-start md:self-auto"
              testId="brochure-download-btn-bottom"
            />
          </div>
        </section>
        <CtaBand
          ...
          text="Share a few details about your home and we'll arrange a conversation at the studio or on site."
